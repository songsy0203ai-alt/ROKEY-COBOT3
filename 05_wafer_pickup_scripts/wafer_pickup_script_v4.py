...
class WaferPickup:
    def __init__(self):
        ...
        self._brown_wafer_position = None

    def setup_scene(self, world: World):
        # 배경 USD 로드
        # background_usd = "/home/rokey/isaacsim/exts/isaacsim.examples.interactive/isaacsim/examples/interactive/hello_world/smcnd_factory_v11.usd"
    ...
    def physics_step(self, step_size):
        ...
        # Phase 2: 짧은 안정화 대기
            ...
            if self._wait_counter < 10:
                ...
                # 버그 수정: 원본의 self.cube → self.wafer
...
async def main():
    ...
    # ── 1단계: 배경 USD만 먼저 스테이지에 추가 ──────────────────
    background_usd = background_usd = "/home/rokey/cobot3_ws/01_digital_twin_map/smcnd_factory_v11.usd"
    add_reference_to_stage(usd_path=background_usd, prim_path="/World/Factory")

    # ── 2단계: reset_async → 이 시점에 스테이지가 확정됨 ─────────
    #    (이 안에서 physics context 초기화가 일어나지만 기존 프림은 유지됨)
    await world.reset_async()

    # ── 3단계: 로봇·wafer 등 나머지 씬 구성 ──────────────────────
    sim.setup_scene(world)          # 배경 추가 라인은 여기서 제거
    await world.reset_async()       # 새 오브젝트 반영을 위한 2차 reset
