...
class WaferPickup:
    ...
    def __init__(self):
        ...
        # 위치 좌표 초기화 설정
        self._wafer_position = np.array([-5.93046331038515, -7.965723514556887, 1.8305293321609353]) # 웨이퍼가 최초 생성되는 고정 좌표
        ...
        # wafer 경로 리스트 추가
        self._wafer_paths = [
            f"/World/Factory/smcnd_factory_v4/wafers/wafer{i}" 
            for i in [1,2,3,4,5,7,8,9,10]  # wafer6 없으므로 제외
        ]
        self._wafer_index = 0  # 현재 사용 중인 wafer 인덱스
    ...
    def setup_scene(self, world: World):
        ...
        # wafer1~10 전부 RigidPrim으로 등록
        from isaacsim.core.prims import XFormPrim

        for i in [1,2,3,4,5,7,8,9,10]:
            world.scene.add(
                XFormPrim(
                    prim_path=f"/World/Factory/smcnd_factory_v4/wafers/wafer{i}",
                    name=f"factory_wafer{i}",
                )
            )

    # 무한반복용
    def _reset_state(self):
        self._wafer_index = (self._wafer_index + 1) % len(self._wafer_paths)
        self._wafer_count = getattr(self, '_wafer_count', 1) + 1

        # 현재 wafer XFormPrim 업데이트
        self.wafer = self.wafer_list[self._wafer_index]
        ...
        # 속도 리셋 (USD RigidBody API 직접 사용)
        stage = omni.usd.get_context().get_stage()
        prim = stage.GetPrimAtPath(self._wafer_paths[self._wafer_index])
        ...
        print(f"[RESET] Wafer #{self._wafer_count} → {self._wafer_paths[self._wafer_index]}")
...
async def main():
    ...
    sim.wafer_list = []
    for i in [1,2,3,4,5,7,8,9,10]:
        sim.wafer_list.append(world.scene.get_object(f"factory_wafer{i}"))

    # 첫 번째 wafer를 현재 타겟으로 설정
    sim.wafer = sim.wafer_list[0]
