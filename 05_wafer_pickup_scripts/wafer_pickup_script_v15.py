...
class WaferPickup:
    ...
    def __init__(self):
        ...
        # --- [추가] ROS 2 초기화 및 Subscriber 노드 생성 ---
        if not rclpy.ok():
            rclpy.init()
        self.ros_node = rclpy.create_node("wafer_pickup_subscriber")
        self.defect_sub = self.ros_node.create_subscription(
            String,
            "/wafer_defect_status",
            self._defect_callback,
            10
        )
        self.current_defect_status = None # 결함 상태 저장 변수

    def _defect_callback(self, msg):
        """ROS 2 외부 노드로부터 결함 탐지 결과를 수신하는 콜백 함수"""
        self.current_defect_status = msg.data
    ...
    def setup_scene(self, world: World):
        ...
        # ── [변경] DynamicCylinder → Mesh Cylinder + OmniPBR 텍스처 ──
        self._create_mesh_wafer(world)


    def _create_mesh_wafer(self, world: World):
        """Mesh Cylinder 생성 후 OmniPBR 머티리얼로 텍스처 적용"""
        import omni.kit.commands
        from pxr import UsdPhysics, PhysxSchema, UsdShade

        stage = omni.usd.get_context().get_stage()
        wafer_path = "/World/wafer"
        texture_path = "/home/rokey/cobot3_ws/04_wafer_deflection_det/wafer_dataset/train/images/d01.png"

        # 1) Mesh Cylinder 생성 (omni.kit.commands 사용)
        omni.kit.commands.execute(
            "CreateMeshPrimWithDefaultXform",
            prim_type="Cylinder",
            prim_path=wafer_path,
        )

        wafer_prim = stage.GetPrimAtPath(wafer_path)

        # 2) 위치 / 스케일 설정
        xform = UsdGeom.Xformable(wafer_prim)
        xform.ClearXformOpOrder()

        pos = self._wafer_position
        xform.AddTranslateOp().Set(Gf.Vec3d(float(pos[0]), float(pos[1]), float(pos[2])))
        xform.AddScaleOp().Set(Gf.Vec3f(0.05, 0.05, 0.05))

        # 3) 물리 속성 추가 (DynamicCylinder 대체용)
        UsdPhysics.RigidBodyAPI.Apply(wafer_prim)
        UsdPhysics.CollisionAPI.Apply(wafer_prim)
        PhysxSchema.PhysxRigidBodyAPI.Apply(wafer_prim)

        mass_api = UsdPhysics.MassAPI.Apply(wafer_prim)
        mass_api.GetMassAttr().Set(0.1)

        # 4) OmniPBR 머티리얼 생성
        mat_path = "/World/Looks/WaferMat"
        omni.kit.commands.execute(
        ...
        if shader_prim.IsValid():
            ...
            # Albedo map 입력 설정
            diffuse_input = shader.GetInput("diffuse_texture")
            if not diffuse_input:
                diffuse_input = shader.CreateInput("diffuse_texture", Sdf.ValueTypeNames.Asset)
            diffuse_input.Set(texture_path)

            # 텍스처가 sRGB임을 명시
            encoding_input = shader.GetInput("encoding")
            if not encoding_input:
                encoding_input = shader.CreateInput("encoding", Sdf.ValueTypeNames.Int)
            encoding_input.Set(0)  # 0 = sRGB

        # 6) 머티리얼을 wafer prim에 바인딩
        UsdShade.MaterialBindingAPI.Apply(wafer_prim)
        material = UsdShade.Material(mat_prim)
        UsdShade.MaterialBindingAPI(wafer_prim).Bind(material)

        print(f"[setup] Mesh wafer created with texture: {texture_path}")

        # 7) Isaac Sim scene에 RigidPrim으로 등록 (get_object로 접근 가능하게)
    ...
    def _reset_state(self):
        ...
        # --- [추가] 다음 웨이퍼를 위해 판독 상태 초기화 ---
        self.current_defect_status = None
    ...
    def _physics_step_impl(self, step_size):
        ...
        # Phase 2: 물리 엔진 안정화 대기 및 외부 결함 판독 결과 대기
        elif self.task_phase == 2:
            if self._wait_counter < 10:
                self._wait_counter += 1
            else:
                # 물리 안정화 후, 외부 노드로부터 판독 결과를 받을 때까지 대기
                if self.current_defect_status is not None:
                    self._brown_wafer_position, _ = self.wafer.get_world_pose()
                    print(f"[Phase 2] Wafer stabilized. Defect status: {self.current_defect_status} → Phase 3")
                    self.task_phase = 3
        ...
        # Phase 7: 목표 배치 위치(Place Position)로 이동 (결함 여부 분기)
        elif self.task_phase == 7:
            print('[Debugging Log] Phase 7 start')  
            
            # --- [수정] 수신된 결함 상태에 따른 목적지 분기 ---
            if self.current_defect_status == "None":
                # 결함 없음 (정상)
                _target_position = np.array([-2.7030928134918213, -9.1062, 2.124995470046997])
            else:
                # 결함 있음 (scratch, donut 등)
                _target_position = np.array([-0.33817, -9.1062, 2.124995470046997])
