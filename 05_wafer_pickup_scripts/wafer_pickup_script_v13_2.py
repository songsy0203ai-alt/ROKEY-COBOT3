...
from pxr import Gf, UsdGeom, UsdLux, Sdf, UsdShade, UsdPhysics
...
from isaacsim.core.api.objects import DynamicCylinder, VisualCylinder, DynamicCuboid
...
class WaferPickup:
    ...
    def setup_scene(self, world: World):
        ...
        # shape) 씬에 픽업 대상인 웨이퍼(동적 원기둥) 객체 추가

        # ===========================================================================================================
        # # 1. DynamicCylinder로 생성 (물리 보장)
        # world.scene.add(
        #     DynamicCylinder(
        ...
        # # 2. Material 생성
        # wafer_material = OmniPBR(
        #     prim_path="/World/Looks/wafer_material",
        #     texture_path="/home/rokey/cobot3_ws/04_wafer_deflection_det/simulation_set/d01_resized.png",
        # )

        # # 3. DynamicCylinder에 material 바인딩
        # stage = omni.usd.get_context().get_stage()
        # mesh_prim = stage.GetPrimAtPath("/World/wafer")
        # UsdShade.MaterialBindingAPI(mesh_prim).Bind(
        #     UsdShade.Material(stage.GetPrimAtPath("/World/Looks/wafer_material"))
        # )
        # ===========================================================================================================
        # world.scene.add(
        #     DynamicCylinder(
        ...
        # # UVW 평면 투영 활성화 ← 핵심
        # wafer_material.set_project_uvw(True)
