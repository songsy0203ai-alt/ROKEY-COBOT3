...
class WaferROS2Node(Node):
    """
    [기능]
      - /wafer_camera/image_raw  퍼블리싱: 맵 내 카메라 영상을 ROS2 Image 메시지로 전송
      - /def_det_result          구독:     외부 YOLO 노드의 분류 결과(none/scratch/donut) 수신
    [설계 의도]
      Isaac Sim physics callback(동기)과 ROS2 spin(비동기)을 분리하기 위해
      WaferROS2Node는 별도 스레드에서 실행된다.
      detection 결과는 스레드 안전하게 self._det_result에 저장되고,
      physics callback에서 읽어간다.
    """
    ...
    def __init__(self):
        ...
        # ── /def_det_result 구독 ────────────────────────────────────
        # 외부 YOLO detection 노드가 분류 결과를 String으로 퍼블리싱한다.
        # 메시지 data 필드: "none" | "scratch" | "donut"
        self._det_result = None          # 가장 최근 detection 결과 저장
        self._det_result_lock = threading.Lock()  # 멀티스레드 안전 접근용
        ...
        # ── /wafer_camera/image_raw 퍼블리셔 ────────────────────────
        # Isaac Sim 카메라 영상을 ROS2 Image 메시지로 전송한다.
        # 외부 YOLO 노드가 이 토픽을 구독하여 detection을 수행한다.
    ...
    def _det_result_callback(self, msg: String):
        """
        [기능] /def_det_result 수신 시 호출되는 콜백.
               결과를 소문자로 정규화하여 저장한다.
        [입력] msg.data: "none" | "scratch" | "donut" (대소문자 무관)
        """
    ...
    def get_det_result(self):
        """
        [기능] 가장 최근 detection 결과를 스레드 안전하게 반환.
        [출력] str | None  — 아직 수신 전이면 None
        """
        with self._det_result_lock:
    ...
    def reset_det_result(self):
        """
        [기능] 다음 웨이퍼 처리 시작 전 이전 detection 결과를 초기화.
        """
        with self._det_result_lock:
    ...
    def publish_camera_image(self, rgb_array: np.ndarray):
        """
        [기능] Isaac Sim 카메라에서 읽은 RGB numpy 배열을
               ROS2 sensor_msgs/Image 메시지로 변환하여 퍼블리싱.
        [입력] rgb_array: shape (H, W, 3), dtype uint8
        """
        msg = Image()
...
class WaferPickup:
    ...
    def _capture_defect_camera(self) -> np.ndarray | None:
        ...
        try:
            ...
            # 카메라 Prim으로 render product 생성 (매번 새로 만들지 않도록
            # 첫 호출 시에만 생성하고 인스턴스에 캐싱한다)
    ...
    def physics_step(self, step_size):
        if self.wafer is None:
            return
        # ROS2 spin_once: physics callback 안에서 ROS2 메시지를 처리한다.
        # spin_once는 non-blocking(timeout=0)으로 호출하여
        # physics 루프를 블로킹하지 않는다.
    ...
    def _physics_step_impl(self, step_size):
        ...
        if self.task_phase == 1:
            ...
            # 전체 대기 카운터 (X 도달 여부 무관하게 계속 증가)
            self._phase1_total_wait = getattr(self, '_phase1_total_wait', 0) + 1

            x_reached = current_x_position >= -1.67499
            ...
            if x_reached:
                if not self._camera_shot_done:
                    ...
                    if rgb is not None:
                        ...
                        print(f"[Phase 1] Camera image published")
                ...
                if self._phase1_det_wait_counter % 60 == 0:
                    ...
                    print(f"[Phase 1] Waiting for detection... ({elapsed}/3 sec)")

                if self._phase1_det_wait_counter >= 90:
                    print(f"[Phase 1] X reached & wait done → Phase 2")
                    self.task_phase = 2

            else:
                # ★ X 미도달 시 타임아웃: 600스텝(10초) 후 강제 진행
                if self._phase1_total_wait % 60 == 0:
                    print(f"[Phase 1] Waiting for X... current={current_x_position:.4f} "
                        f"(need >= -1.67499), elapsed={self._phase1_total_wait} steps")

                if self._phase1_total_wait >= 600:
                    print(f"[Phase 1] TIMEOUT — forcing Phase 2 (X={current_x_position:.4f})")
        ...
        # Phase 7: detection 결과에 따라 place 위치 분기 후 이동
            ...
            if self._resolved_place_pos is None:
                ...
                if det is None or det == "empty":
                    # ── "empty"는 웨이퍼 미탐지 상태이므로 유효한 결과가
                    # 아님. 계속 대기하여 none/scratch/donut을 기다린다.
                    print(f"[Phase 7] Waiting for valid detection (current='{det}') ...")
                    return

                # none / scratch / donut 중 하나가 수신된 경우에만 좌표 확정
                if det == "none":
                    self._resolved_place_pos = PLACE_NORMAL.copy()
                    print(f"[Phase 7] det='{det}' → PLACE_NORMAL {self._resolved_place_pos}")
                else:
                    # scratch 또는 donut → 불량 적재
                    self._resolved_place_pos = PLACE_DEFECT.copy()
