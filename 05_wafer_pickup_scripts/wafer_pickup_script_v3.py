...
class UR10PickAndPlaceTask:
    def __init__(self, world):
        ...
        self._wafer_position = np.array([-2.703093513886345, -8.012578810344934, 2.2778853351258435]) # 웨이퍼 초기 위치 정의
        self.task_phase = 1 # 작업의 현재 단계를 나타내는 변수 (1부터 시작)
        self._wait_counter = 0 # 특정 단계에서의 대기 시간을 계산하는 카운터
        self.robot_position = np.array([-1.69498, -9.02501299679875, 2.1]) # 로봇의 초기 배치 위치
        self.place_position = np.array([-3.69498, -9.02501299679875, 2.1]) # 큐브를 내려놓을 목표 위치
    ...
    async def setup_and_run(self):
        ...
        # 컨베이어 벨트 USD 불러오기 및 Stage에 추가 
        conveyor_usd = "https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Isaac/5.0/Isaac/Props/Conveyors/ConveyorBelt_A06.usd"
        ...
        conveyor_geom.set_local_pose(translation=np.array([-3.0622000674686682, -5.970562922643824, 1.4701799527804797]))
